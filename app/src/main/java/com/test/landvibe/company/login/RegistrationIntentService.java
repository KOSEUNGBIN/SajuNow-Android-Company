...
import com.test.landvibe.company.chat.QuickstartPreferences;
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            synchronized (TAG) {
                ...
                String default_senderId = "{GCM Default Id}";
                // GCM 기본 scope는 "GCM"이다.
                String scope = GoogleCloudMessaging.INSTANCE_ID_SCOPE;
                // Instance ID에 해당하는 토큰을 생성하여 가져온다.
                token = instanceID.getToken(default_senderId, scope, null);
                client.addHeader("Cookie", "PASSWORD");
                param.add("company_no", String.valueOf(intent.getIntExtra("company_no", -1)));
                param.add("company_reg_id", token);
                // DB - update 에 접근
                client.post(getString(R.string.URL) + "/company/update/regid", param, new AsyncHttpResponseHandler() {
                    @Override
